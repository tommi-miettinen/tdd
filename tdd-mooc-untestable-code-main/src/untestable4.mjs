...
const hashPassword = (password) => argon2.hashSync(password);
const verifyPassword = (user, password) => argon2.verifySync(user.passwordHash, password);

export class TestablePasswordService {
  constructor(users) {
    this.users = users;
  }
...
export class TestablePostgresUserDao {
  constructor(db) {
    this.db = db;
  }
